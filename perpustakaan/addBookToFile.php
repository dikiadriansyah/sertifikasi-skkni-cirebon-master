...
if (isset($_POST['insert'])) {
	...
	if(!empty(trim($title)) && !empty(trim($author)) && !empty(trim($publisher)) && !empty(trim($category)) && !empty(trim($year))) {
		if(!file_exists("books.txt")) {
			$myBook = fopen("books.txt", "w") or die("Unable to write file");
		} else {
			$myBook = fopen("books.txt", "a") or die("Unable to read file");
		}

		fwrite($myBook, $title."\n");
		fwrite($myBook, $author."\n");
		fwrite($myBook, $publisher."\n");
		fwrite($myBook, $category."\n");
		fwrite($myBook, $year."\n\n");
		fwrite($myBook, "=================================\n");

		if(fclose($myBook)) {
			echo "<script type='text/javascript'>
					alert('New book was successfully inserted to File!');
				</script>";
		} else {
			echo "<script type='text/javascript'>alert('Failed to write data into file!')</script>";
		}
	}
}
...
<body>
	<h1>- Library App -</h1>
	<p class="center">
		Add Book Data to File.
		<hr>
	</p>
	<form action="addBookToFile.php" method="POST">
		<table style="width: 600px !important;margin:0 auto;" cellpadding="5">
			...
			<tr>
				...
				<td>
					...
					<button type="button" onclick="window.location='readFile.php';" name="back" class="inp-btn">Read File</button>
				</td>
				<td></td>
			</tr>
		</table>
